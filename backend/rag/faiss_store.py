...
class FaissJobStore:
    def __init__(self, job_id: str, index_dir: str | None = None, dim: int = EMBEDDING_DIM) -> None:
        ...
        self.index_path = self.index_dir / f"job_{job_id}.index"
        self.metadata_path = self.index_dir / f"job_{job_id}.metadata.json"
        self.vectors_path = self.index_dir / f"job_{job_id}.vectors.npy"
    ...
    def load(self) -> None:
        ...
        if faiss is not None and self.index_path.exists():
            ...
            logger.info("[INFO] FAISS index loaded | job_id=%s | vectors=%s", self.job_id, self.index.ntotal)
            return
        if self.vectors_path.exists():
            self.vectors = np.load(self.vectors_path).astype(np.float32)
            logger.info("[INFO] Fallback vector store loaded | job_id=%s | vectors=%s", self.job_id, len(self.vectors))
    ...
    def add(self, chunks: List[Dict[str, Any]], embeddings: np.ndarray) -> None:
        ...
        logger.info("[SUCCESS] FAISS index updated | total_vectors=%s", total)
